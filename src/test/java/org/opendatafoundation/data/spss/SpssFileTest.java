...
public class SpssFileTest {
	...
	public void testReadingShortStringVariableSmallerValue() {
		try {
			...
			SPSSStringVariable stringVariable = (SPSSStringVariable) variable;
			String variableValue = stringVariable.data.get(0); // getValueAsString()
																// pads value
																// based on
																// ASCII format
	...
	public void testReadingLongStringVariableOtherTypesInBetween() {
		try {
			...
			// you a complete account of the s";
	...
	public void testSYSMISS() {
		...
		try {
			spssFile = new SPSSFile(
					new File("src/test/resources/org/opendatafoundation/data/spss/OV103V2OVERALLv5b.sav"));
			spssFile.loadMetadata();
			spssFile.loadData();
		} catch (Exception e) {
	...
	public void testNumericValueFormat() {
		...
		try {
			...
			assertThat(variable.getValueAsString(7, new FileFormatInfo(FileFormatInfo.Format.ASCII)),
					is("123456789.34"));
